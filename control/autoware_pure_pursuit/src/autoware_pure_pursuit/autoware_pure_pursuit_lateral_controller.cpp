  ...
  const double target_curvature, const double target_velocity)
{
  ...
  const double v_abs = std::abs(v);
  // Pure tracking only. In-place rotation is handled by trajectory_follower state machine.
  const double omega_raw = (v_abs < param_.min_velocity_for_angular_rate) ? 0.0 : (v * target_curvature);
  const double omega =
